...
use tc_bluetooth::{BluetoothBackend, BluetoothCommand, BluetoothEvent, BluetoothRuntime};
use tc_call_system::{CallSystemBackend, CallSystemCommand, CallSystemEvent, CallSystemEventSink};
use tc_core::{ModuleCommandEnvelope, ModuleEventEnvelope};
use tc_location::{LocationBackend, LocationCommand, LocationEvent, LocationEventSink};
use tc_notifications::{
    NotificationCommand, NotificationEvent, NotificationsBackend, NotificationsEventSink,
};
use tc_peer_transport::{
    PeerTransportBackend, PeerTransportRuntime, TransportCommand, TransportEvent,
};
use tc_ranging::{RangingBackend, RangingCommand, RangingEvent, RangingEventSink};
use tc_secure_storage::{
...
mod tests {
    ...
    use tc_bluetooth::{BluetoothBackendError, BluetoothCapabilities, BluetoothEventSink};
    use tc_call_system::{CallSystemBackendError, CallSystemCapabilities};
    use tc_location::{LocationBackendError, LocationCapabilities};
    use tc_notifications::{NotificationCapabilities, NotificationsBackendError};
    use tc_peer_transport::{
        PeerTransportBackendError, PeerTransportEventSink, TransportCapabilities,
    };
    use tc_ranging::{RangingBackendError, RangingCapabilities};
    use tc_secure_storage::{SecureStorageBackendError, SecureStorageCapabilities};
    ...
    probe_backend!(
        ...
        "peerTransport",
        {
            ...
            fn send_frame(
                ...
                _request_id: String,
                _connection: u64,
                _channel: tc_peer_transport::TransportChannel,
                _bytes: Vec<u8>,
            ) -> Result<(), PeerTransportBackendError> {
                self.probe.operation();
                Ok(())
    ...
    probe_backend!(
        ...
        "callSystem",
        {
            ...
            fn set_route(
                &self,
                _request_id: String,
                _route: tc_call_system::AudioRoute,
            ) -> Result<(), CallSystemBackendError> {
                self.probe.operation();
                Ok(())
            }
