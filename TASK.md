...
travel-companion/
  ...
  modules/                           自包含的跨平台系统能力
    ...
    notifications/                本地通知能力；Apple/Android backend 自包含
    call-system/                  CallKit/AVFoundation 与 Android 对应 backend
    secure-storage/               Keychain 与 Android Keystore backend

  crates/                            纯 Rust 领域与基础设施逻辑
    model/                        peer/group/event/resource 等共享值类型
    crypto/                       签名、AEAD、KDF 和防重放 primitive
    group-auth/                   PAKE/PIN 入群、成员凭据、epoch 和密钥轮换
    protocol/                     wire message、版本、framing 和编解码
    store/                        SQLite 事件、索引、游标、投影和迁移
    replication/                  复制、relay、anti-entropy 和 delivery tracking
    resources/                    manifest、哈希、分块、续传和垃圾回收
    location-logic/               GPS/UWB 组合、距离方位、stale 和降级逻辑
    im/                           群聊/私聊及消息 materialization
    document/                     Trip.md revision、lease 和冲突副本
    call/                         通话信令和通话状态机
    travel-core/                  统筹全部 crate/module 的应用核心

  bindings/
    app-ffi/                      面向 SwiftUI/Kotlin GUI 的唯一公共 UniFFI 门面
...
每个 `modules/*` 是一个逻辑上的 polyglot package，而不只是普通 Cargo crate。以 `bluetooth` 为例：
...
| --- | --- | --- | --- |
| `bluetooth` | BLE 扫描、连接、GATT、控制消息收发 | Core Bluetooth | BluetoothGatt |
| `peer-transport` | 本地点对点发现、连接、数据与实时通道 | Network framework、Bonjour、AWDL | 按 Android 实验选择 |
| `location` | GPS/位置采样、缓存、按需请求、后台能力 | Core Location | Android location API |
| `ranging` | 近距离精确测距与可选方向 | Nearby Interaction | Android UWB API |
| `notifications` | 本地通知的发布、合并和点击事件 | UserNotifications | Android notifications |
| `call-system` | 系统来电、音频会话和路由事件 | CallKit、AVFoundation | Android Telecom/audio |
| `secure-storage` | 小型密钥和凭据的安全读写 | Keychain | Android Keystore |
...
`travel-core` 是 Rust 应用层唯一协调者，负责开始/结束旅行、群组、复制、定位、消息、文档和通话状态，并组合各能力 module。`app-ffi` 是面向 GUI 的唯一公共 UniFFI 门面：SwiftUI 主 App 通常只看到一个 `TravelCore`，不直接操作各内部 Rust crate 或 module binding。

```text
SwiftUI App
  -> app-ffi / TravelCore
  -> travel-core
     -> bluetooth       <-> module 私有 FFI <-> Swift Core Bluetooth backend
     -> peer-transport  <-> module 私有 FFI <-> Swift Network framework backend
     -> location        <-> module 私有 FFI <-> Swift Core Location backend
     -> ranging         <-> module 私有 FFI <-> Swift Nearby Interaction backend
     -> replication / im / document / ...（纯 Rust）
```
...
Cargo 只负责编译 Rust，SwiftPM/Xcode 负责编译 Swift，Gradle 以后负责编译 Kotlin。顶层使用 `xtask` 或等价构建脚本统一完成 binding 生成、各平台 backend 编译、静态库/XCFramework 组装和测试，不假设单独运行 `cargo build -p bluetooth` 就能编译完整 Apple/Android package。
...
- `travel-core` 只统筹用例和长期服务生命周期，不实现各平台 framework 细节；SwiftUI view 只观察 `TravelCore` 快照或事件流；
...
- module 私有 FFI 与面向 GUI 的 `app-ffi` 分离；内部 crate 重构不应迫使 SwiftUI 页面修改；
- BLE 控制面与本地高速数据面必须通过统一 peer identity 关联；BLE 只承载小型控制提示，事件正文和资源始终走数据面；
- `replication` 是针对少量设备、机会式本地网络裁剪的复制型事件日志，不实现通用分布式数据库、全局事务、选主或强一致共识；
- 所有可同步业务修改均表示为不可变事件；创建、编辑和撤回分别发布事件并引用同一实体 ID，feature 不直接把可变数据库行发送给其他成员；
- 事件发布必须先在本机事务性分配 sender sequence、签名并落盘，成功后再交给 `replication` 分发；原始事件日志是同步事实来源，聊天消息、地点和文档等 UI 状态由 materialization 生成；
