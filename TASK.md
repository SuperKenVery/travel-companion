...
正式版采用 **Rust-first multi-crate workspace + 纵向自包含的跨平台能力模块 + SwiftUI 主 App**。复杂业务状态机、协议、安全、复制与持久化尽量放在 Rust；Swift 负责 GUI、Apple App 生命周期，以及各能力模块内部的 Apple framework backend。

蓝牙、近场高速传输、定位、UWB、通知、通话系统和安全存储分别作为独立的 **polyglot module**。每个 module 以 idiomatic Rust API 对外，内部可以包含 Rust、UniFFI/C ABI、Swift 和将来的 Kotlin 代码；调用者只依赖能力语义，不知道实际由 Core Bluetooth、Network framework、Core Location、Nearby Interaction 或 Android 对应 API 实现。
...
travel-companion/
  ...
  modules/                           自包含的跨平台系统能力
    tc-bluetooth/
      Cargo.toml                     对外 Rust API、命令/事件、BLE 状态机
      src/
      apple/                         Swift + Core Bluetooth backend
      android/                       Kotlin + BluetoothGatt backend（后续）
      ffi/                           module 私有的 UniFFI/C ABI 边界

    tc-peer-transport/
      Cargo.toml                     本地点对点连接、数据/实时通道、重连
      src/
      apple/                         Swift + Network framework/Bonjour/AWDL
      android/                       Android 本地高速传输 backend（后续）
      ffi/

    tc-location/
      Cargo.toml                     普通位置采样、缓存、按需刷新和能力模型
      src/
      apple/                         Swift + Core Location backend
      android/                       Android location backend（后续）
      ffi/

    tc-ranging/
      Cargo.toml                     精确测距请求与 session 状态机
      src/
      apple/                         Swift + Nearby Interaction backend
      android/                       Android UWB backend（后续）
      ffi/

    tc-notifications/                本地通知能力；Apple/Android backend 自包含
    tc-call-system/                  CallKit/AVFoundation 与 Android 对应 backend
    tc-secure-storage/               Keychain 与 Android Keystore backend

  crates/                            纯 Rust 领域与基础设施逻辑
    tc-model/                        peer/group/event/resource 等共享值类型
    tc-crypto/                       签名、AEAD、KDF 和防重放 primitive
    tc-group-auth/                   PAKE/PIN 入群、成员凭据、epoch 和密钥轮换
    tc-protocol/                     wire message、版本、framing 和编解码
    tc-store/                        SQLite 事件、索引、游标、投影和迁移
    tc-replication/                  复制、relay、anti-entropy 和 delivery tracking
    tc-resources/                    manifest、哈希、分块、续传和垃圾回收
    tc-location-logic/               GPS/UWB 组合、距离方位、stale 和降级逻辑
    tc-im/                           群聊/私聊及消息 materialization
    tc-document/                     Trip.md revision、lease 和冲突副本
    tc-call/                         通话信令和通话状态机
    tc-core/                         统筹全部 crate/module 的应用核心

  bindings/
    tc-app-ffi/                      面向 SwiftUI/Kotlin GUI 的唯一公共 UniFFI 门面

  app-ios/
    TravelCompanion/                 SwiftUI、导航、权限说明和 App 生命周期装配

  app-android/                       后续 Android GUI
```

### 12.1 跨平台能力 module

每个 `modules/tc-*` 是一个逻辑上的 polyglot package，而不只是普通 Cargo crate。以 `tc-bluetooth` 为例：

- 对外暴露平台无关、符合 Rust 使用习惯的 `Bluetooth`、`BluetoothCommand`、`BluetoothEvent`、`PeerHandle` 和 capability API；
- Rust 调用者不接触 `CBCentralManager`、`CBPeripheral`、Android `BluetoothGatt` 或任何 Swift/Kotlin 对象；
- Apple backend 在 module 自己的 `apple/` 目录中注册 Core Bluetooth delegate、保存 Apple 对象并把回调转换为普通事件；
- Android backend 以后在同一 module 的 `android/` 目录中提供相同能力语义；
- module 可以独立进行 Rust 状态机测试、Apple 真机集成测试和 Android contract test，也可脱离本 App 复用。

其他系统能力遵循相同结构：

| Rust module | 平台无关语义 | Apple backend | Android backend（后续） |
| --- | --- | --- | --- |
| `tc-bluetooth` | BLE 扫描、连接、GATT、控制消息收发 | Core Bluetooth | BluetoothGatt |
| `tc-peer-transport` | 本地点对点发现、连接、数据与实时通道 | Network framework、Bonjour、AWDL | 按 Android 实验选择 |
| `tc-location` | GPS/位置采样、缓存、按需请求、后台能力 | Core Location | Android location API |
| `tc-ranging` | 近距离精确测距与可选方向 | Nearby Interaction | Android UWB API |
| `tc-notifications` | 本地通知的发布、合并和点击事件 | UserNotifications | Android notifications |
| `tc-call-system` | 系统来电、音频会话和路由事件 | CallKit、AVFoundation | Android Telecom/audio |
| `tc-secure-storage` | 小型密钥和凭据的安全读写 | Keychain | Android Keystore |

公共 API 统一产品所需要的能力语义，但不强行抹平平台差异。各 module 必须提供 capability snapshot；Apple 或 Android 独有能力可以通过显式扩展表达，上层不得假设所有平台行为完全相同。

### 12.2 module 内部 FFI

module 内部允许使用 UniFFI foreign trait/callback interface 或经过审计的 C ABI vtable，使 Rust 主动向 Swift/Kotlin backend 发命令，并由 native callback 把结果送回 Rust。命令和事件采用异步 request ID 模型：

```text
Rust module
  -> connect(commandID, peerHandle)
  -> Swift/Kotlin backend 立即返回，不等待系统结果
  -> Apple/Android framework 异步回调
  -> connected(commandID, peerHandle) 或 failed(commandID, error)
  -> Rust module 更新状态并唤醒 event stream
```

跨 module FFI 只传普通值、字节和不透明整数 handle，不传 `CBPeripheral`、`NWEndpoint`、`CLLocation`、`NISession`、Swift `Data` 或 JVM 对象。native backend 自己持有并释放平台对象，Rust 只保存稳定 handle。

Rust 不直接静态依赖某个 Swift/Kotlin 实现符号；平台启动时通过 foreign object 或函数表注册 backend。这样纯 Rust crate 可以在 macOS/Linux 上接入 fake backend 测试，且不会因缺少 Apple/Android 符号而无法链接。

### 12.3 顶层核心与 GUI FFI

`tc-core` 是 Rust 应用层唯一协调者，负责开始/结束旅行、群组、复制、定位、消息、文档和通话状态，并组合各能力 module。`tc-app-ffi` 是面向 GUI 的唯一公共 UniFFI 门面：SwiftUI 主 App 通常只看到一个 `TravelCore`，不直接操作各内部 Rust crate 或 module binding。

```text
SwiftUI App
  -> tc-app-ffi / TravelCore
  -> tc-core
     -> tc-bluetooth       <-> module 私有 FFI <-> Swift Core Bluetooth backend
     -> tc-peer-transport  <-> module 私有 FFI <-> Swift Network framework backend
     -> tc-location        <-> module 私有 FFI <-> Swift Core Location backend
     -> tc-ranging         <-> module 私有 FFI <-> Swift Nearby Interaction backend
     -> tc-replication / tc-im / tc-document / ...（纯 Rust）
```

iOS App 入口保留一个很薄的装配层，负责初始化各 Apple backend、创建 `TravelCore`，并把启动、前后台、通知点击和终止等系统生命周期事件传入相应 module。具体 framework 实现仍保存在各 module 内，不集中到 App 或一个巨型 Apple adapter 中。

### 12.4 构建与产物

Cargo 只负责编译 Rust，SwiftPM/Xcode 负责编译 Swift，Gradle 以后负责编译 Kotlin。顶层使用 `xtask` 或等价构建脚本统一完成 binding 生成、各平台 backend 编译、静态库/XCFramework 组装和测试，不假设单独运行 `cargo build -p tc-bluetooth` 就能编译完整 Apple/Android package。

iOS 主 App 最终只应导入稳定的公共核心 binding 和 Apple modules 的 bootstrap 产物；module 私有生成代码不作为 GUI 业务 API。Rust panic 不得越过 FFI，所有 buffer、callback、foreign object 和 shutdown 都必须有明确所有权与释放规则。

架构约束：

- `tc-core` 只统筹用例和长期服务生命周期，不实现各平台 framework 细节；SwiftUI view 只观察 `TravelCore` 快照或事件流；
- 每个跨平台能力 module 拥有自己的 Rust API、native backend 和私有 FFI，调用者只依赖能力语义；不得建立一个集中实现所有 Apple framework 的巨型 adapter；
- module 的 Rust 公共 API 不出现 Apple/Android 类型；跨 FFI 的平台对象一律由 native backend 保存并以稳定 handle 引用；
- module 内部命令不得同步等待 Apple/Android framework 完成；所有异步结果通过带 request ID 的事件返回，支持超时、取消和去重；
- module 私有 FFI 与面向 GUI 的 `tc-app-ffi` 分离；内部 crate 重构不应迫使 SwiftUI 页面修改；
- BLE 控制面与本地高速数据面必须通过统一 peer identity 关联；BLE 只承载小型控制提示，事件正文和资源始终走数据面；
- `tc-replication` 是针对少量设备、机会式本地网络裁剪的复制型事件日志，不实现通用分布式数据库、全局事务、选主或强一致共识；
- 所有可同步业务修改均表示为不可变事件；创建、编辑和撤回分别发布事件并引用同一实体 ID，feature 不直接把可变数据库行发送给其他成员；
- 事件发布必须先在本机事务性分配 sender sequence、签名并落盘，成功后再交给 `tc-replication` 分发；原始事件日志是同步事实来源，聊天消息、地点和文档等 UI 状态由 materialization 生成；
- 逻辑群组广播由多次点对点复制、成员设备存储转发和重连后的 anti-entropy 共同完成，不假设底层存在可靠广播；
- 每个 publication 按发布时的 group epoch 和 audience 固化目标成员集合，并逐成员追踪确认；只有目标设备验证并持久化后才算 `delivered`，relay 持有副本不能显示为目标已送达；
- 同步摘要使用“每个发送者的最大连续 sequence + 精确稀疏缺口”；持久化确认可由其他成员转发，使原发送者最终获知间接送达结果；
- delivery tracking 至少区分本机已落盘、relay 已持有、各目标已持久化、全部完成和策略过期；消息编辑与撤回事件拥有各自独立的送达进度；
- Rust 侧的连接、复制、资源传输、定位、UWB 和通话状态分别隔离；native framework 对象固定在各 backend 要求的 queue/actor 上，不假定其 `Send`/`Sync`；
- UWB 距离和方向作为两个可独立缺失的能力建模；距离可用但方向缺失时继续展示精确距离，方向明确回退到带来源标识的 GPS 结果，不能保留旧 UWB 箭头；
- 协议从第一天带显式 `protocolVersion`，但首版不实现旧协议兼容；
- 所有系统时间、无线电和传输层均可替换为 fake backend，以便在桌面环境做确定性测试并模拟丢包、乱序、重连和网络分区。
