...
3. **能力不满足时明确拒绝运行**：设备缺少 Wi‑Fi Aware 或 UWB 时展示原因，不静默切换到另一套旧实现。
...
- 设备必须同时满足：
  - `WACapabilities` 表明设备支持所需 Wi‑Fi Aware 能力；
  - `NISession.deviceCapabilities` 支持 UWB 精确测距；
  - Bluetooth、Wi‑Fi、Nearby Interaction 和定位权限可用。
...
### 4.1 高速数据面：Wi‑Fi Aware + Network framework

采用 iOS 26 的 Wi‑Fi Aware 和新版 Network framework：

- `DeviceDiscoveryUI`：App-to-App 设备发现和系统配对；
- `NetworkBrowser`：发现已配对的旅行伴侣设备；
...
Wi‑Fi Aware 负责高吞吐、低延迟的数据传输，但不被假设为能够单独、持续唤醒一个已挂起的 App。
...
`dataAvailable` 只表示“某成员的本地事件日志已有新内容”，不承载消息正文。接收方被 BLE 事件唤醒后，使用自己的同步游标通过 Wi‑Fi Aware 主动拉取缺失事件；这样文字、图片、语音、文档和其他事件始终共用一套可靠内容传输与确认逻辑。
...
- 创建群组时生成 group ID、群组 epoch 和群组密钥；
- 加入者通过 `DeviceDiscoveryUI` 与邀请者配对；
- 双方完成应用层握手，确认群组名称、成员身份和短校验码；
- 群组密钥使用接收者公钥封装，不以明文广播；
...
- `Network.framework` 只用于连接 Wi‑Fi Aware 本地 peer endpoint，不允许回退到公网连接或基于基础设施网络的 Bonjour；
- 邀请者是新成员的首个本地点对点连接，不是服务器；
- 已完成系统配对的成员之间尽量建立直接 Wi‑Fi Aware 连接；
...
所有需要“发给群内成员”的功能统一依赖 `GroupReplicationEngine`，feature 不自行遍历连接、管理 ACK 或实现重传。Wi‑Fi Aware 没有被当成底层广播网络；逻辑群组广播由多次点对点复制、成员手机存储转发和重连后的 anti-entropy 共同实现。
...
5. 任意两台设备建立 Wi‑Fi Aware 连接时先交换 digest，再双向请求缺失区间；同步不依赖某条 BLE 提示一定到达。
...
消息提醒采用“BLE 敲门，Wi‑Fi Aware 取内容”的流程：
...
5. 接收方主动建立对应 peer 的 Wi‑Fi Aware `NetworkConnection`，发送自己的同步 cursor，并拉取 cursor 之后缺失的事件；
6. 优先拉取文字、事件元数据和资源 manifest；图片原件、长语音等资源使用分块协议继续传输，不要求在一次后台窗口中完成；
7. 拉取消息内容成功后生成包含发送者和允许展示的摘要的本地通知；如果后台时间不足或 Wi‑Fi Aware 暂时连接失败，则生成不含正文的“有新消息”通知；
...
3. 双方建立直接 Wi‑Fi Aware `NetworkConnection`；
...
在全面开发 UI 前，直接在正式 App 中完成第一个端到端 vertical slice，并提供仅 Debug 构建可见的诊断页面。优先验证两台真机之间的建群、BLE 提示、Wi‑Fi Aware 内容同步和位置响应；无需建立独立 target 或强制编写正式报告。关键测试仍需在开发日志或自动化测试产物中记录设备、系统版本、权限状态、成功率、延迟和能耗，以便据此冻结后台策略。

### 11.1 Wi‑Fi Aware 与新版 Network API

- 两台和四台真机完成 App-to-App 配对；
- 验证多连接、断线重连和设备离开/回来；
- 测量小消息延迟、大文件吞吐和 realtime/bulk 模式能耗；
- 验证连接全部使用 Wi‑Fi Aware 路径，未意外依赖互联网或同一无线路由器；
...
- 验证 `dataAvailable` 唤醒后建立 Wi‑Fi Aware 连接、按 cursor 拉取文字消息并生成本地通知的完整流程；
...
- 群聊、私聊和文字消息；
- 图片和语音消息；
- BLE `dataAvailable`、Wi‑Fi Aware cursor 拉取、本地通知和资源断点续传。

### M4：语音通话

- BLE 来电信令；
- CallKit 与音频会话；
- Wi‑Fi Aware 实时音频、重连和中断处理。
...
- [ ] 后台接收方可由经过认证的 BLE `dataAvailable` 获得处理机会，并通过 Wi‑Fi Aware 拉取消息内容。
...
- Apple Watch 距离提示；
- 旅行结束后的加密导出、备份和设备迁移；
- 在不破坏简洁性的前提下评估跨平台 Wi‑Fi Aware 客户端。
...
- [Wi‑Fi Aware 支持无接入点的安全点对点连接](https://developer.apple.com/documentation/wifiaware)
- [Apple 的 Wi‑Fi Aware App-to-App 示例](https://developer.apple.com/documentation/wifiaware/building-peer-to-peer-apps)
