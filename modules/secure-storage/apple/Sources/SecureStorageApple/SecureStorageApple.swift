...
public enum SecureStorageEvent: Sendable, Equatable {
...
public typealias SecureStorageEventSink = @Sendable (SecureStorageEvent) -> Void

/// Platform capability values exposed without leaking Security.framework objects.
public struct SecureStorageCapabilitySnapshot: Sendable, Equatable {
...
public actor SecureStorageAppleBackend {
    public nonisolated static var capabilitySnapshot: SecureStorageCapabilitySnapshot {
    ...
    private let eventSink: SecureStorageEventSink
    private let service = "com.travelcompanion.credentials"

    public init(eventSink: @escaping SecureStorageEventSink) {
