...
public enum TcLocationAuthorization: Sendable, Equatable {
    case notDetermined
    case denied
    case whenInUse
    case always
...
public struct TcLocationSample: Sendable, Equatable {
...
public enum TcLocationEvent: Sendable, Equatable {
    case started(requestID: String)
    case stopped(requestID: String)
    case authorizationChanged(status: TcLocationAuthorization)
    case sample(requestID: String?, sample: TcLocationSample, fromCache: Bool)
    case timedOut(requestID: String, staleSample: TcLocationSample?)
    case failed(requestID: String?, code: String, message: String, retryable: Bool)
}

public typealias TcLocationEventSink = @MainActor @Sendable (TcLocationEvent) -> Void

/// Platform capability values exposed without leaking Core Location objects.
public struct TcLocationCapabilitySnapshot: Sendable, Equatable {
...
public final class TcLocationAppleBackend {
    public nonisolated static var capabilitySnapshot: TcLocationCapabilitySnapshot {
        TcLocationCapabilitySnapshot(
    ...
    private var latestSample: TcLocationSample?
    ...
    @discardableResult
    private func consume(_ location: CLLocation, stationary: Bool) -> TcLocationSample? {
        guard
    ...
    private func emit(_ event: TcLocationEvent) {
        eventSink(event)
    }
