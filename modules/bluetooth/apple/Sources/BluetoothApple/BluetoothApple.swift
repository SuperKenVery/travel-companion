...
public typealias BluetoothEventSink = @MainActor @Sendable (BluetoothEvent) -> Void

/// Platform capability values exposed without leaking CoreBluetooth objects.
public struct BluetoothCapabilitySnapshot: Sendable, Equatable {
...
@MainActor
public final class BluetoothAppleBackend: NSObject {
    ...
    private static let centralRestoreID = "com.travelcompanion.bluetooth.central"
    private static let peripheralRestoreID = "com.travelcompanion.bluetooth.peripheral"
    private nonisolated static let maximumPacketBytes = 180

    public nonisolated static var capabilitySnapshot: BluetoothCapabilitySnapshot {
        BluetoothCapabilitySnapshot(
            central: true,
            peripheral: true,
    ...
    private let eventSink: BluetoothEventSink
    ...
    public init(eventSink: @escaping BluetoothEventSink) {
    ...
    private func emit(_ event: BluetoothEvent) {
        eventSink(event)
    }
...
private extension Error {
    var isRetryableBluetoothError: Bool {
        guard let error = self as? BluetoothAppleBackend.BackendError else { return false }
        switch error {
...
extension BluetoothAppleBackend: @preconcurrency CBPeripheralDelegate {
