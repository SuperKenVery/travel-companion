...
public enum PeerTransportChannel: Int, Sendable, Equatable {
    case control = 1
    case event = 2
    case chunk = 3
    case audio = 4
...
public enum PeerTransportConnectionSource: Sendable, Equatable {
    case inbound
    case outbound
}

public enum PeerTransportEvent: Sendable, Equatable {
    case discoveryStarted(requestID: String)
    case discoveryStopped(requestID: String)
    case peerFound(peerID: String)
    case connectionOpened(
        connection: UInt64,
        source: PeerTransportConnectionSource,
        expectedPeerID: String?
    )
    case disconnected(connection: UInt64, reason: String)
    case frameReceived(
        connection: UInt64,
        channel: PeerTransportChannel,
        bytes: [UInt8]
    )
...
public typealias PeerTransportEventSink = @Sendable (PeerTransportEvent) -> Void

/// Platform capability values exposed without leaking Network.framework objects.
public struct PeerTransportCapabilitySnapshot: Sendable, Equatable {
...
public actor PeerTransportAppleBackend {
    private static let serviceType = "_travel._tcp"

    public nonisolated static var capabilitySnapshot: PeerTransportCapabilitySnapshot {
        PeerTransportCapabilitySnapshot(
            localOnly: true,
            peerToPeer: true,
    ...
    private let eventSink: PeerTransportEventSink
    ...
    public init(eventSink: @escaping PeerTransportEventSink) {
    ...
    private func handle(
        ...
        do {
            for try await (payload, metadata) in connection.messages {
                ...
                guard let channel = PeerTransportChannel(rawValue: metadata.type) else {
    ...
    private func emit(_ event: PeerTransportEvent) {
        eventSink(event)
    }
